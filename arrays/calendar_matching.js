...
function getMatchingAvailabilties(calendar,meetingDuration){
	const matchingAvailabilties = [];
	for(let i = 1; i< calendar.length; i++){
		const start = calendar[i-1][1];
		const end = calendar[i][0];
		const availabilityDuration = end - start ;
    if(availabilityDuration >= meetingDuration){
			matchingAvailabilties.push([start,end]);
		} 
	}
	return matchingAvailabilties.map(meeting => meeting.map(minutesToTime));
}
